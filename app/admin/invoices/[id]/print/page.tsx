      ...
      <div style={{ maxWidth: 794, margin: "0 auto", padding: "40px 48px", background: "#fff", minHeight: "100vh" }}>
        ...
        <div style={{ backgroundColor: headerColor, padding: "24px 32px", display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 40, color: "#F3F2EE" }}>
          <div>
            {logoUrl ? (
              ...
              <div style={{ fontSize: 22, fontWeight: 700, fontFamily: headingFont, marginBottom: 8 }}>
                {fromEntity.name ?? "StaatWright"}
              </div>
            )}
            {fromEntity.address && (
              <div style={{ fontSize: 12, opacity: 0.8, whiteSpace: "pre-line", lineHeight: 1.5 }}>{fromEntity.address}</div>
            )}
            {fromEntity.reg_number && (
              <div style={{ fontSize: 11, opacity: 0.7, marginTop: 4 }}>Reg: {fromEntity.reg_number}</div>
            )}
            {fromEntity.vat_number && (
              <div style={{ fontSize: 11, opacity: 0.7 }}>VAT: {fromEntity.vat_number}</div>
            )}
            {fromEntity.email && (
              <div style={{ fontSize: 11, opacity: 0.7 }}>{fromEntity.email}</div>
            )}
          </div>

          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 13, opacity: 0.7, fontFamily: "sans-serif", marginBottom: 4 }}>INVOICE</div>
            ...
            <div style={{ fontSize: 12, opacity: 0.8, lineHeight: 1.8, fontFamily: "sans-serif" }}>
              ...
              <div style={{ marginTop: 4 }}>
                <span style={{ backgroundColor: doc.status === "paid" ? "#22c55e" : doc.status === "overdue" ? "#ef4444" : "#f59e0b", padding: "2px 10px", fontSize: 11, borderRadius: 0, textTransform: "uppercase", letterSpacing: 1, color: "#fff", fontWeight: 600 }}>
                  {doc.status}
                </span>
              </div>
            </div>
        ...
        {doc.partner && (
          <div style={{ marginBottom: 40 }}>
            <div style={{ fontSize: 10, fontFamily: "sans-serif", textTransform: "uppercase", letterSpacing: 2, color: "#5C6E81", marginBottom: 8, fontWeight: 600 }}>Bill To</div>
            ...
            {(doc.line_items ?? []).map((item, idx) =>
              item.type === "heading" ? (
                <tr key={item.id} style={{ backgroundColor: "#EAE4DC" }}>
                  <td colSpan={5} style={{ padding: "10px 12px", fontWeight: 700, fontFamily: "sans-serif", fontSize: 13, color: "#1F2A38" }}>
                    {item.description}
                  </td>
                </tr>
              ) : (
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 40 }}>
          <div style={{ width: 280, fontFamily: "sans-serif" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "12px 0", fontSize: 16, fontWeight: 700, color: "#1F2A38" }}>
              <span>TOTAL DUE</span><span>{formatZAR(doc.total ?? 0)}</span>
            </div>
          </div>
        </div>
